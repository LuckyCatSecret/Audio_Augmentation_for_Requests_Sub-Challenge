...
def rand_window(size,lam):
    ...
    cut_w=np.int(W*cut_rat)
    cx=np.random.randint(W)
    windowx1=np.clip(cx-cut_w//2,0,W)
    windowx2=np.clip(cx+cut_w//2,0,W)
    return windowx1,windowx2

####### mix vision2 tag #######
# def wave_mixup(wave1,wave2,v):
    # assert 0.0<=v<=1.0
def wave_mixup(wave1,wave2):
    ####### mix vision2 tag #######
    v = np.random.beta(10, 10)
    if wave1.shape[1] != wave2.shape[1]:
        if wave1.shape[1] > wave2.shape[1]:
            # padding
            # temp_wav = torch.zeros(1, wave1.shape[1])
            # temp_wav[0, 0:wave2.shape[1]] = wave2
            # wave2 = temp_wav
            p = wave1.shape[1] - wave2.shape[1]
            while p > 0:
                op = wave2.shape[1]
                wave2 = F.pad(wave2.unsqueeze(0), (0, min(p, wave2.shape[1])), mode='circular').squeeze(0)
                p -= min(p, op)
        else:
            # cutting
            wave2 = wave2[0, 0 : wave1.shape[1]]

    wave=v*wave1+(1.0-v)*wave2
    wave=wave-wave.mean()
    # return wave
    return wave,v

####### mix vision2 tag #######
# def wave_cutmix(wave1,wave2,v):
    # assert 0.0 <= v <= 1.0
def wave_cutmix(wave1,wave2):
    v = np.random.beta(10, 10)
    if wave1.shape[1] != wave2.shape[1]:
        if wave1.shape[1] > wave2.shape[1]:
            # padding
            ...
            # temp_wav[0, 0:wave2.shape[1]] = wave2
            # wave2 = temp_wav
            # wave2 = F.pad(wave2.unsqueeze(0), (0, wave1.shape[1] - wave2.shape[1]), mode='circular').squeeze(0)
            p = wave1.shape[1] - wave2.shape[1]
            while p > 0:
                op = wave2.shape[1]
                wave2 = F.pad(wave2.unsqueeze(0), (0, min(p, wave2.shape[1])), mode='circular').squeeze(0)
            ...
            # cutting
            wave2 = wave2[:, 0:wave1.shape[1]]
    # print(wave2.shape)
    w1,w2=rand_window(wave2.shape[1],v)
    wave1[0,w1:w2]=wave2[0,w1:w2]
    # return wave1
    ####### mix vision2 tag #######
    return wave1,v

####### mix vision2 tag #######
# def fbank_mixup(fbank1,fbank2,v):
    # assert 0.0 <= v <= 1.0
def fbank_mixup(fbank1,fbank2):
    v = np.random.beta(10, 10)
    fbank=v*fbank1+(1.0-v)*fbank2
    # return fbank
    return fbank,v
####### mix vision2 tag #######

####### mix vision2 tag #######
# def fbank_cutmix(fbank1,fbank2,v):
    # assert 0.0 <= v <= 1.0
def fbank_cutmix(fbank1,fbank2):
    v = np.random.beta(10, 10)
    bbx1,bby1,bbx2,bby2=rand_bbox(fbank2.size(),v)
    fbank1[bbx1:bbx2,bby1:bby2]=fbank2[bbx1:bbx2,bby1:bby2]
    # return fbank1
    return fbank1,v
####### mix vision2 tag #######
...
def augment_mix_list():
    l=[
        (wave_mixup,0.0,1.0), #0
        (wave_cutmix,0.0,1.0), #1
        (fbank_mixup,0.0,1.0), #2
        (fbank_cutmix,0.0,1.0), #3
    ]
    return l
...
augment_mix_dict={fn.__name__:(fn,v1,v2) for fn,v1,v2 in augment_mix_list()}
...
def get_mix_augment(name):
    return augment_mix_dict[name]
...
def apply_mix_augment(name):
    augment_fn, low, high = get_mix_augment(name)
    return augment_fn