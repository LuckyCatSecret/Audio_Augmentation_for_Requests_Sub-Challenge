...
def eval_tta(config, augment, reporter):
    ...
    # model = models_vit.__dict__[amt_conf.model](
    #     num_classes=amt_conf.nb_classes,
    #     drop_path_rate=amt_conf.drop_path,
    #     global_pool=amt_conf.global_pool,
    #     mask_2d=amt_conf.mask_2d,
    #     use_custom_patch=amt_conf.use_custom_patch,
    ...
    losses = np.array(losses)
    audio_output = torch.cat(A_predictions).cpu().numpy()
    # mix up会出现小数，计算ap值不支持小数
    target = torch.cat(A_targets).cpu().numpy()
    target = target + 0.5
...
### 封装为search policy
def search_policy(copied_c,path,amtconfig,aim_map,num_search):
    import json
    ...
    for _ in range(1):  # run multiple times.
        for cv_fold in range(cv_num):
            ...
            for result in results[:num_result_per_cv]:
                ...
                if result.last_result['mAP']>=aim_map*0.8 and result.last_result['mAP']<=aim_map*1.2:
