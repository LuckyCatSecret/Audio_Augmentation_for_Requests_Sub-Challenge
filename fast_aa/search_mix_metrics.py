...
# from .archive import remove_deplicates, policy_decoder
from .archive_mix import remove_deplicates, policy_decoder
...
from .augmentations_mix import augment_wav_list,augment_spec_list,augment_cv_list,augment_mix_list
...
# from FastAutoAugment.data import get_dataloaders
# from .data import get_dataloaders
from .data_mix import get_dataloaders
...
def step_w_log(self):
    ...
    for trial in filter(lambda x: x.status == Trial.TERMINATED, self._trials):
        ...
        best_top1_mAP = max(best_top1_mAP, trial.last_result['metrics'])
    print('iter', self._iteration, 'top1_metrics=%.3f' % best_top1_mAP, cnts, end='\r')
...
def eval_tta(config, augment, reporter):
    ...
    # for _ in range(1):  # TODO
        ...
        _, tl, validloader, tl2 = get_dataloaders(amt_conf,C.get()['dataset'],C.get()['batch'], augment['dataroot'], cv_ratio_test, split_idx=cv_fold)
    ...
    try:
        while True:
            for loader in loaders:
                ...
                label = label.cuda()
                loss = loss_fn(audio_output.detach().cpu(), label.detach().cpu())
    ...
    # target = torch.cat(A_targets)+0.5
    ...
    if augment['metrics'] == 'acc':
        acc = stats[0]['acc']
        logger.info(f'{augment["metrics"]}={acc}')
        metrics.add_dict({
            # 'minus_loss': -1 * np.sum(losses_min),
            'mean_loss': np.mean(losses),
            'metrics': acc,
        })
    elif augment['metrics'] == 'uar':
        uar = stats[0]['UAR']
        logger.info(uar)
        metrics.add_dict({
            # 'minus_loss': -1 * np.sum(losses_min),
    ...
    reporter(minus_loss=metrics['mean_loss'], metrics=metrics['metrics'], elapsed_time=gpu_secs, done=True)
    return metrics['metrics']

### 封装为search policy
def search_policy(copied_c,path,amtconfig,aim_map,num_search, metrics, cv_ratio):
    import json
    from pystopwatch2 import PyStopwatch
    w = PyStopwatch()

    print(f"num_search = {num_search}, metrics = {metrics}, cv_ratio = {cv_ratio}")
    
    # ray.init(local_mode=True)
    ray.init()
    num_result_per_cv = num_search
    cv_num = 1
...
####### mix vision2 tag #######
    for i in range(4):
        space['mix_policy_%d_%d' % (i, 0)] = hp.choice('mix_policy_%d_%d' % (i, 0), list(range(0, len(mix_ops))))
        space['mix_prob_%d_%d' % (i, 0)] = hp.uniform('mix_prob_%d_ %d' % (i, 0), 0.0, 1.0)
    # for i in range(4):
    #     space['mix_policy_%d_%d' % (i, 0)] = hp.choice('mix_policy_%d_%d' % (i, 0), list(range(0, len(mix_ops))))
    #     space['mix_prob_%d_%d' % (i, 0)] = hp.uniform('mix_prob_%d_ %d' % (i, 0), 0.0, 1.0)
    #     space['mix_level_%d_%d' % (i, 0)] = hp.uniform('mix_level_%d_ %d' % (i, 0), 0.0, 1.0)
    ...
    final_policy_set = []
    total_computation = 0
    reward_attr = 'metrics'      # top1_valid or minus_loss
    for _ in range(1):  # run multiple times.
        for cv_fold in range(cv_num):
            name = "search_%s_%s_fold%d_ratio%.1f" % (C.get()['dataset'], C.get()['model']['type'], cv_fold, cv_ratio)#args.cv_ratio=0.2
            print(name)
            ...
            exp_config = {
                name: {
                    'run': name,
                    'num_samples': num_search,#args.num_search=20
                    'resources_per_trial': {'gpu': 1},
                    'stop': {'training_iteration': 5},#args.num_policy=5
                    'config': {
                        'save_path': path,
                        'cv_ratio_test': cv_ratio, #args.cv_ratio=0.2
                        'cv_fold': cv_fold,
            ...
            # results = run_experiments(exp_config, search_alg=algo, scheduler=None, verbose=0, queue_trials=True, resume="LOCAL", raise_on_failed_trial=False)   #resume=args.resume='store_true'=False
            # print()
            ...
            for result in results[:num_result_per_cv]:
                ...
                logger.info('loss=%.12f metrics=%.4f %s' % (result.last_result['minus_loss'], result.last_result['metrics'], final_policy))
                if result.last_result['metrics']>=aim_map*0.9 and result.last_result['metrics']<=aim_map*1.1:
