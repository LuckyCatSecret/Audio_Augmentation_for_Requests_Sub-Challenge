...
from .augmentations_mix import apply_wav_augment,apply_spec_augment,apply_cv_augment,apply_mix_augment
...
class AudioDataset_finetue(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None,aug_wav=None,aug_spec=None,aug_cv=None,aug_mix=None):
        """
        Dataset that manages audio recordings
        :param audio_conf: Dictionary containing the audio loading and preprocessing settings
        :param dataset_json_file
    ...
    def _wavef2fbank(self,waveform,sr):
        ...
        if p > 0:
            ...
            while pp > 0:
                op = fbank.shape[0]
                fbank = torch.nn.functional.pad(fbank.unsqueeze(0).unsqueeze(0), (0, 0, 0, min(fbank.shape[0], pp)), mode='circular').squeeze(0).squeeze(0)
                pp -= min(pp, op)
        elif p < 0:
            fbank = fbank[0:target_length, :]
        return fbank
    ...
    def __getitem__(self,index):
        ...
        waveform2, sr = torchaudio.load(mix_datum['wav'])
        # waveform2 = waveform2[1].unsqueeze(0)
        waveform2=waveform2-waveform2.mean()

        ### add mix
        if self.aug_mix is not None:
            ####### mix vision2 tag #######
            fn,pr,mix_fname=self.aug_mix()
            # fn,pr,level,mix_fname=self.aug_mix()
            ####### mix vision2 tag #######
            randpr=random.random()

        ####### stage4-1 augment for wave mix and deal label index########
        # if 'wave' in mix_fname and randpr<=pr:
        if self.aug_mix is not None and 'wave' in mix_fname and randpr<=pr:
            waveform,lamb=fn(waveform.clone(),waveform2)
            # waveform=fn(waveform.clone(),waveform2,level)
            ####### mix vision2 tag #######
            for label_str in datum['labels'].split(','):
                label_indices[int(self.index_dict[label_str])] += lamb
                # label_indices[int(self.index_dict[label_str])] += level
                ####### mix vision2 tag #######
            # add sample 2 labels
            for label_str in mix_datum['labels'].split(','):
                label_indices[int(self.index_dict[label_str])] += (1.0-lamb)
                # label_indices[int(self.index_dict[label_str])] += (1.0-level)
                ####### mix vision2 tag #######
            label_indices = torch.FloatTensor(label_indices)
        ...
        fbank2=self._wavef2fbank(waveform2,sr)  #waveform2 without aug
        ...
        ####### stage4-2 augment for fbank mix and deal label index########
        # if 'fbank' in mix_fname and randpr<=pr:
        if self.aug_mix is not None and 'fbank' in mix_fname and randpr<=pr:
            fbank,lamb=fn(fbank.clone(),fbank2)
            # fbank=fn(fbank.clone(),fbank2,level)
            ####### mix vision2 tag #######
            for label_str in datum['labels'].split(','):
                label_indices[int(self.index_dict[label_str])] += lamb
                # label_indices[int(self.index_dict[label_str])] += level
        ...
        # deal labels, if we don't mix-augment
        for label_str in datum['labels'].split(','):
            label_indices[int(self.index_dict[label_str])] = 1.0
        label_indices = torch.FloatTensor(label_indices)
...
def build_loader_finetune(dataset_json_file,audio_conf,drop_last=True,shuffle=True,label_csv=None,aug_wav=None,aug_spec=None,aug_cv=None,aug_mix=None):
    dataset=AudioDataset_finetue(dataset_json_file,audio_conf,label_csv,aug_wav,aug_spec,aug_cv,aug_mix)
    sampler=DistributedSampler(dataset,num_replicas=dist.get_world_size(),rank=dist.get_rank(),shuffle=shuffle)
    dataloader=DataLoader(dataset,audio_conf.get("batch_size"),sampler=sampler,num_workers=16,pin_memory=True,drop_last=drop_last,collate_fn=collate_fn)
    return dataset,dataloader
...
def make_mix_aug(final_policy_set_mix):
    aug_mix=MIXAugmentation(final_policy_set_mix)
    return aug_mix
...
class MIXAugmentation(object):
    ...
    def __call__(self):
        for _ in range(1):
            ...
            # print("----------------------------------------------------------")
            for name,pr in policy:
                return apply_mix_augment(name),pr,name
        #         fn=apply_mix_augment(name)
        #         pr=pr
        #         name=name
        # return fn,pr,name
            # for name,pr,level in policy:
            #     return apply_mix_augment(name),pr,level,name
            ####### mix vision2 tag #######
