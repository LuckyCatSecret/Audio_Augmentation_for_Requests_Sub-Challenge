...
from engine_inference_as import train_one_epoch, evaluate #, train_one_epoch_av, evaluate_av
#######
# from dataset import AudiosetDataset, DistributedWeightedSampler, DistributedSamplerWrapper
from fast_aa.amtaa_dataloader_mix import build_loader_finetune,make_wav_aug,make_spec_aug,make_cv_aug,make_mix_aug
import torch.distributed as dist
from fast_aa.search_mix import search_policy
import copy
...
def get_args_parser():
    ...
    parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["audioset", "esc50", "speechcommands", "k400", "hcc"])
    ...
    parser.add_argument("--final_policy", type=str, default="", help="final_policy") 
    parser.add_argument("--save_file_name", type=str, default="", help="final_policy") 
...
def main(args):
    ...
    C.get()['data'] = {}
    C.get()['dataset'] = args.dataset
    C.get()['batch'] = args.batch_size
    C.get()['model'] = {}
    C.get()['model']['type'] = args.model
    ...
    target_length = {'audioset':1024, 'k400':1024, 'esc50':512, 'speechcommands':128,'hcc':3096}
    multilabel_dataset = {'audioset': True, 'esc50': False, 'k400': False, 'speechcommands': True,'hcc':True}
    ...
    # final_policy_path = "/YourPath/HC-C/AudioMAE-aa/hcc_complaint_policy.json"
    final_policy_path = args.final_policy
    print(f"final_policy_path = {final_policy_path}")
    with open(final_policy_path, 'r') as load_f:
        final_policy_set = json.load(load_f)
    print(f"final_policy_set {final_policy_set}")
    wav_policy_set=list()
    spec_policy_set=list()
    # cv_policy_set=list()
    mix_policy_set=list()
    for i in range(len(final_policy_set)//3):
        wav_policy_set=wav_policy_set+final_policy_set[3*i]
        spec_policy_set=spec_policy_set+final_policy_set[3*i+1]
        # cv_policy_set=cv_policy_set+final_policy_set[4*i+2]
        mix_policy_set=mix_policy_set+final_policy_set[3*i+2]
    aug_wav=make_wav_aug(wav_policy_set,audio_conf_train.get('sample_rate'))
    aug_spec=make_spec_aug(spec_policy_set)
    # aug_cv=make_cv_aug(cv_policy_set)
    aug_mix=make_mix_aug(mix_policy_set)
    if len(final_policy_set) == 0:
        aug_wav = None
        aug_spec = None
        aug_cv = None
        aug_mix = None
    dataset_train,data_loader_train=build_loader_finetune(args.data_train,audio_conf_train,True,True,args.label_csv,aug_wav,aug_spec,None,aug_mix)
    dataset_val,data_loader_val=build_loader_finetune(args.data_eval,audio_conf_val,False,False,args.label_csv,None,None,None,None)
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, args.dist_eval, args.save_file_name, args.task_name)
    ...
    ####### modify for iteration search #######
    # search_point = [0.4, 0.6, 0.8, 0.9]
    # search_epoch_point = [int(args.epochs * i) for i in search_point]
    # save_path_point = [i - 1 for i in search_epoch_point]
    # search_numbers = [50, 30, 20, 20]
    # cnt = 0
    # saved_path="/YourPath/noaug-bestmodel.pth"
    # final_policy_path='final_policy_search.json'
    ####### modify for iteration search #######
    # config = get_config(args)
    # if torch.distributed.get_rank()==0:
        # final_policy_set = search_policy(copied_c, saved_path, args, 0.3329, 200)
        # with open(final_policy_path, 'w') as write_f:
            # write_f.write(json.dumps(final_policy_set, ensure_ascii=False))
    for epoch in range(args.start_epoch, args.epochs):
    #     ####### situation 1 in search_epoch_point, start search #######
    #     if epoch > 0 and epoch in search_epoch_point:
    #     # if epoch == 1:
    #         del data_loader_train, data_loader_val, dataset_train, dataset_val
    #         if dist.get_rank() == 0:
    #             print(f"{dist.get_rank()} start search.")
    #             print(f"saved_path={saved_path}")
    #             # if epoch==10:
    #             # final_policy_set=search_policy(copied_c,saved_path,config,max_accuracy,30)
    #             # else:
    #             # final_policy_set=search_policy(copied_c,saved_path,config,max_accuracy,25)
    #             print(f"search number = {search_numbers[cnt]}")
    #             final_policy_set = search_policy(copied_c, saved_path, args, max_mAP, search_numbers[cnt])
    #             cnt += 1
    #             # if len(final_policy_set)>0 or epoch==10:
    #             if len(final_policy_set) > 0:
    #                 with open(final_policy_path, 'w') as write_f:
    #                     write_f.write(json.dumps(final_policy_set, ensure_ascii=False))
    #         else:
    #             print(f"{dist.get_rank()} sleep.")
    #             pass
    #         torch.distributed.barrier()
    #         # 0->1-6
    #         # torch.distributed.broadcast_object_list(final_policy_set, 0)
    #         # torch.distributed.barrier()
    #         with open(final_policy_path, 'r') as load_f:
    #             final_policy_set = json.load(load_f)
    #         print(f"{dist.get_rank()}, policy is {final_policy_set}")
    #         wav_policy_set = list()
    #         spec_policy_set = list()
    #         cv_policy_set = list()
    #         mix_policy_set = list()
    #         for i in range(len(final_policy_set) // 4):
    #             wav_policy_set = wav_policy_set + final_policy_set[4 * i]
    #             spec_policy_set = spec_policy_set + final_policy_set[4 * i + 1]
    #             cv_policy_set = cv_policy_set + final_policy_set[4 * i + 2]
    #             mix_policy_set = mix_policy_set + final_policy_set[4 * i + 3]
    #         aug_wav = make_wav_aug(wav_policy_set, audio_conf_train.get('sample_rate'))
    #         aug_spec = make_spec_aug(spec_policy_set)
    #         aug_cv = make_cv_aug(cv_policy_set)
    #         aug_mix = make_mix_aug(mix_policy_set)
    ...
    #             aug_cv = None
    #             aug_mix = None
    #         dataset_train, data_loader_train = build_loader_finetune(args.data_train, audio_conf_train, True, True,
    #                                                                  args.label_csv, aug_wav, aug_spec, aug_cv, aug_mix)
    #         dataset_val, data_loader_val = build_loader_finetune(args.data_eval, audio_conf_val, False, False,
    #                                                              args.label_csv, None, None, None, None)
        ...
        if args.dataset in ["voc-c","ksf-c", "hcc"]:
            eval_metrics="uar"
        elif args.dataset in ["esc50","deepship"]:
            eval_metrics="acc"
        else:
            eval_metrics = "mAP"
        ...
        if args.output_dir:
            saved_path = misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch)
        if epoch >= args.first_eval_ep:
            test_stats = evaluate(data_loader_val, model, device, args.dist_eval, args.save_file_name)
            print(f"mAP of the network on the {len(dataset_val)} test images: {test_stats['mAP']:.4f}")
            max_mAP = max(max_mAP, test_stats["mAP"])
            ###add
            max_UAR = max(max_UAR, test_stats["UAR"])
            max_acc=max(max_acc,test_stats["acc"])
            print(f'Max mAP: {max_mAP:.4f}')
            ###add
            print(f'Max UAR: {max_UAR:.4f}') 
            print(f'Max acc: {max_acc:.4f}')
            if eval_metrics == "uar":
                if max_UAR == test_stats["UAR"] and args.output_dir:
                    print(f"max_UAR = {max_UAR} and saved model")
                    misc.save_best_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch)
            elif eval_metrics == 'acc':
                if max_acc == test_stats["acc"] and args.output_dir:
                    misc.save_best_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch)
            else:
                if max_mAP == test_stats["mAP"] and args.output_dir:
                    misc.save_best_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch)
        else:
