...
@torch.no_grad()
def evaluate(data_loader, model, device, dist_eval, save_file_name, task_name):
    ...
    results = [['filename','prediction','TRUE']]
    if task_name == 'complaint':
        for i in range(len(vids)):
            results.append([vids[i].split('/')[-1], comlaint_dict[outputs[i]], comlaint_dict[targets[i]]])
    else:
        for i in range(len(vids)):
            results.append([vids[i].split('/')[-1], request_dict[outputs[i]], request_dict[targets[i]]])
