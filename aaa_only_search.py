...
from fast_aa.search_mix_metrics import search_policy
import copy
...
def get_args_parser():
    ...
    parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["audioset", "esc50", "speechcommands", "k400","deepship","voc-c","ksf-c","hcc"])
    ...
    parser.add_argument('--saved_path', default='', help='resume from checkpoint')
    parser.add_argument('--final_policy', default='', help='resume from checkpoint')
...
def main(args):
    ...
    audio_conf_train = {'num_mel_bins': 128, 
                    'target_length': target_length[args.dataset], 
                    'freqm': aug_param[args.dataset][0],
                    'timem': aug_param[args.dataset][1],
                    'mixup': aug_param[args.dataset][2],
                    'dataset': args.dataset,
                    'mode':'train',
                    'mean':norm_stats[args.dataset][0],
                    'std':norm_stats[args.dataset][1],
    ...
    # final_policy_path = "/YourPath/final_policy.json"
    ...
    if torch.distributed.get_rank()==0:
        print(f"search hcc fold{args.fold}, max_accuracy = {acc[args.fold]}")
        # final_policy_set = search_policy(copied_c, saved_path, args, acc[args.fold], 120, 'acc', 0.2)
        final_policy_set = search_policy(copied_c, saved_path, args, acc[args.fold], 200, 'uar', 0.99)
