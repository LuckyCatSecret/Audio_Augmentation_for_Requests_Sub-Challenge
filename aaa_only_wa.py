...
def main(args):
    ...
    final_policy_path = "/YourPath/final_policy_search.json"
    ...
    start_ckeckpoint = 119
    end_ckeckpoint = 1

    sdA = torch.load(f'/YourPath/output/checkpoint-{start_ckeckpoint}.pth', map_location='cpu')['model']
    start_ckeckpoint -= 1
    model_cnt = 1
    max_mAP = 0
    while start_ckeckpoint >= end_ckeckpoint:
        print(f"load checkpoint {start_ckeckpoint}")
        sdB = torch.load(f'/YourPath/output/checkpoint-{start_ckeckpoint}.pth', map_location='cpu')['model']
        for key in sdA:
            sdA[key] = sdA[key] + sdB[key]
        model_cnt += 1
        if start_ckeckpoint <= 119:
            sd = torch.load(f'/YourPath/output/checkpoint-119.pth', map_location='cpu')['model']
            for key in sd:
                sd[key] = sdA[key] / float(model_cnt)
            model.module.load_state_dict(sd)
            model.to(device)
            print(f"Start wa test")
